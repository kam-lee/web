$(document).ready(function () {
    ...
    var pre_path = ["tag/fac/", "tag/staf/", "tag/jump/"];
    //获取文件内容
    ...
    async function fetchAllFoldersData() {
        ...
        const folderIndices = Array.from({
            length: num
        }, (_, i) => i + 1);
    ...
    async function MAIN() {
        ...
        //更新intro内容
        var num_fac = obj_tmp["fac"];
        var num_staf = obj_tmp["staf"];
        var num_doc = obj_tmp["doc"];
        var num_mast = obj_tmp["mast"];
        var num_stu = num_mast + num_doc;
        ...
        const dateString = `${monthNames[monthIndex]} ${year}`;
        $(".intro").html("As of " + dateString +
                         ", HPL has <span style=\"font-size:40px\">" + num_fac.toString() + 
                         "</span> faculties, <span style=\"font-size:40px\">" + num_staf.toString() + 
                         "</span> staff, and <span style=\"font-size:40px\">" + num_stu.toString() + 
                         "</span> graduate students, including <span style=\"font-size:40px\">" + num_doc.toString() + 
                         "</span> Ph.D. candidates and <span style=\"font-size:40px\">" + num_mast.toString() + 
                         "</span> master's students.");
        //获取内容
